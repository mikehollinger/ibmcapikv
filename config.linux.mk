...
define PROGRAMS_template
 $(1)_PGM_OFILES = $(addprefix ${PGMDIR}/, $($(notdir $(1)_OFILES))) $(1).o
 $(1): $$($(1)_PGM_OFILES) $(1).o $(notdir $(1)).c
...
define BIN_TESTS_template
 $(1)_BTEST_OFILES = $(addprefix ${TESTDIR}/, $($(notdir $(1)_OFILES))) $(1).o
 $(1): $$($(1)_BTEST_OFILES) $(1).o $(notdir $(1)).c
...
define GTESTS_template
 GTEST_DEPS = $(TESTDIR)/gtest-all.o $(TESTDIR)/gtest_main.o $(TESTDIR)/gtest_main.a
 $(1)_GTESTS_OFILES = $(addprefix $(TESTDIR)/,$($(notdir $(1))_OFILES)) $(1).o
 $(1): $$(GTEST_DEPS) $$($(1)_GTESTS_OFILES) $(1).o $(notdir $(1)).C
...
define GTESTS_NM_template
 GTEST_NM_DEPS = $(TESTDIR)/gtest.a
 $(1)_GTESTS_NM_OFILES = $(addprefix $(TESTDIR)/,$($(notdir $(1))_OFILES)) $(1).o
 $(1): $$(GTEST_NM_DEPS) $$($(1)_GTESTS_NM_OFILES) $(1).o $(notdir $(1)).C
 ALL_OFILES += $$($(1)_GTESTS_NM_OFILES) $(1)
endef
$(foreach _gtest_nm,$(GTESTS_NM),$(eval $(call GTESTS_NM_template,$(_gtest_nm))))
...
DEPS += $(addsuffix .dep, ${BIN_TESTS}) $(addsuffix .dep, ${PROGRAMS}) \
        ...
        $(addsuffix .dep, ${GTESTS_NM})

BEAMOBJS  = $(addprefix ${BEAMDIR}/, ${OBJS:.o=.beam})
GENTARGET = $(addprefix %/, $(1))

${PROGRAMS} ${BIN_TESTS} ${GTESTS_DIR} $(GTESTS_NM) ${OBJECTS} ${LIBRARIES}: makefile
${LIBRARIES}: ${OBJECTS}
${EXTRA_PARTS} ${PROGRAMS}: ${LIBRARIES}
$(GTESTS_DIR) $(GTESTS_NM) $(BIN_TESTS): $(GTEST_TARGETS)

dep:       ${SUBDIRS:.d=.dep} ${DEPS} ${ROOTPATH}/src/include/cflash_ioctl.h
code_pass: ${SUBDIRS} ${LIBRARIES} ${EXTRA_PARTS} ${PROGRAMS}
test:      ${SUBDIRS:.d=.test} ${BIN_TESTS} ${GTESTS_DIR} ${GTESTS_NM}
fvt:       ${SUBDIRS:.d=.fvt}
unit:      ${SUBDIRS:.d=.unit}
