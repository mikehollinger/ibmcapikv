...
char             *cblk_log_filename;           /* Trace log filename       */
                                               /* This traces internal     */
                                               /* activities               */
int              cblk_log_verbosity = 0;       /* Verbosity of traces in   */
                                               /* log.                     */
FILE             *cblk_logfp;                  /* File descriptor for      */
                                               /* trace log                */
...

{
	...
	if ( (ulong)chunk & CHUNK_BAD_ADDR_MASK ) {
	    ...
	    CBLK_TRACE_LOG_FILE(1,"Corrupted chunk address = 0x%llx, index = 0x%x", 
				chunk, (chunk_id & CHUNK_HASH_MASK));
...
void _cflsh_blk_init(void)
{
    ...
    /*
     * TODO ?? We need to disable this time-out overidding via environment
     * variables when we ship
     */
				   ...
				   cblk_arw_status_t *status)
{
	...
	pthread_rc = pthread_cond_wait(&(chunk->resume_event),&(chunk->lock.plock));
    ...
    *cmd_index = cmd->index;
     ...
     *       CDBs are using 512 alignment (thus we are multipling
     *       all requests by 8). Long term we need to reconcile
     *       this.
     */
    
    
    CFLASH_BUILD_RW_16(cdb,(chunk->start_lba + lba)*cflsh_blk.blk_size_mult,nblocks*cflsh_blk.blk_size_mult);
	...
	async_data = &(cmd->async_data);
	async_data->chunk = chunk;
	async_data->cmd_index = cmd->index;
	
	pthread_rc = pthread_create(&(cmd->thread_id),NULL,cblk_async_recv_thread,async_data);
	...
	if (lib_flags & CFLASH_ASYNC_OP) {
	    ...
	    pthread_rc = pthread_cancel(cmd->thread_id);
		 ...
		 * cleared cmd->in_use. Thus the command
		 ...
		 */
		CFLASH_BLOCK_UNLOCK(chunk->lock);
		pthread_join(cmd->thread_id,NULL);
		CFLASH_BLOCK_LOCK(chunk->lock);
    ...
    if (op_code == SCSI_READ_16) {
        ...
        cmd->flags |= CFLSH_MODE_READ;
	    
    } else if (op_code == SCSI_WRITE_16) {

        cmd->flags |= CFLSH_MODE_WRITE;
...
chunk_id_t cblk_open(const char *path, int max_num_requests, int mode, chunk_ext_arg_t ext, int flags)
{
    ...
    CBLK_TRACE_LOG_FILE(5,"opening %s with max_num_requests = %d and flags = 0x%x",
			path,max_num_requests,flags);
    ...
    if (cflsh_blk.num_active_chunks) {
	    ...
	    /*
	     * If this caller is requesting a
	     * virtual lun, but we have already
	     * allowed another chunk to use whole
	     * physical lun, then fail this request.
	     */

            CBLK_TRACE_LOG_FILE(1,"Attempting to open a virtual lun when a physical lun is in use");
	    ...
	    /*
	     * If this caller is requesting the
	     * full physical lun, but we have already
	     * allowed another chunk to use a virtual
	     * lun, then fail this request.
	     */

            CBLK_TRACE_LOG_FILE(1,"Attempting to open a physical lun when a virtual lun is in use");
    ...
    chunk_type = cblk_get_chunk_type(path);

    ret_chunk_id = cblk_get_chunk(CFLSH_BLK_CHUNK_SET_UP, max_num_requests,chunk_type);
...
#ifdef BLOCK_FILEMODE_ENABLED
	open_flags = mode & O_ACCMODE;   /* ??TODO Try without O_CLOEXEC */
	...
	/*
	 * TODO: ?? Need to add extended open flag. maybe based on flags
	 *       use NO_RESERVE or break reservations.
	 */
...
int cblk_get_lun_size(chunk_id_t chunk_id, size_t *nblocks, int flags)
{
    ...
    *nblocks = cflsh_blk.num_blocks_lun;
...
int cblk_read(chunk_id_t chunk_id,void *buf,cflash_offset_t lba, size_t nblocks, int flags)
{
    ...
    errno = 0;
    CBLK_TRACE_LOG_FILE(5,"lba = 0x%llx, nblocks = 0x%llx",lba,(uint64_t)nblocks);
		...
		cblk_arw_status_t *status, int flags)
{
    ...
    cflsh_chunk_t *chunk;
    int cmd_index = 0;
    //unused var currently


    errno = 0;

    CBLK_TRACE_LOG_FILE(6,"lba = 0x%llx, nblocks = 0x%llx",lba,(uint64_t)nblocks);
...
int cblk_aresult(chunk_id_t chunk_id,int *tag, uint64_t *status, int flags)
{
    ...
    *status = 0;
	...
	rc = CBLK_WAIT_FOR_IO_COMPLETE(chunk,tag,&transfer_size,FALSE);
	...
	while (cmdi) {
		...
		(chunk->cmd_start[cmdi->index].flags & CFLSH_ASYNC_IO) &&
		(chunk->cmd_start[cmdi->index].state == CFLSH_MGM_CMP)) {
	...
	if (*tag == -1) {
		...
		CFLASH_BLOCK_UNLOCK(chunk->lock);
	        ...
	        rc = CBLK_WAIT_FOR_IO_COMPLETE(chunk,tag,&transfer_size,TRUE);
	...
	if (cmd->flags & CFLSH_ASYNC_IO) {
	    ...
	    // TODO ?? Can this happen for async I/O if so is this a bug?
	 ...
	 */
	 ...
	 if (cmd->thread_id) {
	     ...
	     pthread_rc = pthread_detach(cmd->thread_id);
		 ...
		 CBLK_TRACE_LOG_FILE(5,"pthread_detach failed rc = %d errno = %d, cmd_index = %d,lba = 0x%llx, num_active_cmds = 0x%x, num_active_threads = 0x%x",
				pthread_rc,errno, cmd->index,cmd->lba,chunk->num_active_cmds,chunk->stats.num_active_threads);
		...
		uint64_t timeout,int flags)
{
    ...
    CBLK_TRACE_LOG_FILE(6,"issue_items = 0x%x, pending_items = 0x%x, wait_items = 0x%x, timeout = 0x%llx,flags = 0x%x",
			issue_items,pending_items,wait_items,timeout,flags);
    ...
    *completion_items = 0;
	...
	for (i=0; i< issue_items;i++) {
		...
		rc = cblk_aread(chunk_id,io->buf,io->lba,io->nblocks,&(io->tag),&(io->stat),io_flags);
	    } else {

		rc = cblk_awrite(chunk_id,io->buf,io->lba,io->nblocks,&(io->tag),&(io->stat),io_flags);
	    }
 ...
 *
 * FUNCTION:    clone a chunk with an existing (original)
