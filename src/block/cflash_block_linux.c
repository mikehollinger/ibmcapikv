...
int  cblk_chunk_attach_process_map (cflsh_chunk_t *chunk, int mode, int *cleanup_depth)
{
    ...
    // in SISLite p 7.
    afu_mmio_write_dw(p_afu, 10, (uint64_t)chunk->p_hrrq_start); // START_EA
    afu_mmio_write_dw(p_afu, 12, (uint64_t)chunk->p_hrrq_end);   // END_EA
    ...
    rc = ioctl(chunk->fd,CXL_IOCTL_GET_PROCESS_ELEMENT,&chunk->contxt_handle);
	...
	chunk->mmap_size = CFLASH_MMIO_CONTEXT_SIZE;
    } else {
	/*
    ...
    *cleanup_depth = 40;   
	...
	chunk->mmio = chunk->mmio_mmap + CFLASH_MMIO_CONTEXT_SIZE * chunk->contxt_handle;
     ...
     * returned chunk->mmio_mapp address.
     */
    chunk->mmio = chunk->mmio_mmap + CFLASH_MMIO_CONTEXT_SIZE * chunk->contxt_handle;
    ...
    if (CBLK_SETUP_BAD_MMIO_SIGNAL(chunk,port_lun_table_start+8)) {
    ...
    CBLK_TRACE_LOG_FILE(6,"chunk->mmio = 0x%llx, port_lun_table_start = 0x%llx",chunk->mmio,port_lun_table_start);


    out_mmio64 (chunk->mmio + port_lun_table_start, cblk_lun_id);
					...
					int *cleanup_depth)
{
	...
	sprintf(chunk->master_name,"%s",chunk->dev_name);


	if (chunk->dev_name[end-1] == 's') {
	    chunk->master_name[end-1] = '\0';
	} 
	strcat(chunk->master_name,"m");
    } else {
    ...
    rc = mc_register(chunk->master_name,chunk->contxt_handle,chunk->mmio,
		&(chunk->master.mc_handle));

    if (rc) {
	
	CBLK_TRACE_LOG_FILE(1,"mc_register failed with rc = %d, errno = %d for path = %s",
			    rc, errno,chunk->master_name);
	return -1;
    }
    ...
    rc = mc_open(chunk->master.mc_handle,MC_RDWR,&(chunk->master.sisl.resrc_handle));
	 ...
	 */
	rc = mc_unregister(chunk->master.mc_handle);
	...
	rc = mc_stat(chunk->master.mc_handle,chunk->master.sisl.resrc_handle,&mstat);
	...
	chunk->master.mc_page_size = 1 << mstat.nmask;

	if (chunk->master.mc_page_size == 0) {
...
int  cblk_chunk_set_mc_size(cflsh_chunk_t *chunk, size_t nblocks)
{
	...
	if ((nblocks < chunk->master.num_blocks) &&
	    ((chunk->master.num_blocks * cflsh_blk.blk_size_mult) >= chunk->master.mc_page_size) &&
	    (nblocks > (chunk->master.num_blocks - (chunk->master.mc_page_size/cflsh_blk.blk_size_mult)))) {
	     ...
	     *       (ie. 512, or 4K) and num_blocks is in units of 4K.  cflsh_blk.blk_size_mult
	...
	if ((nblocks * cflsh_blk.blk_size_mult) % chunk->master.mc_page_size) {

	    /*
	     * The size is not divisible by chunk->master.mc_page_size (i.e.
	    ...
	    size =  (nblocks * cflsh_blk.blk_size_mult)/chunk->master.mc_page_size + 1;
	} else {
	    ...
	    size = (nblocks * cflsh_blk.blk_size_mult)/chunk->master.mc_page_size;
	}
    } else {
    ...
    rc = mc_size(chunk->master.mc_handle,chunk->master.sisl.resrc_handle,
		 size,&actual_size);
     ...
     * size (cflsh_blk.blk_size_mult is the multiple to go from the device's block
     * size to 4K) and and our num_blocks is in units of 4K block sizes we need convert this 
     * size into 4K blocks.  
     */

    chunk->master.num_blocks = (actual_size * chunk->master.mc_page_size)/cflsh_blk.blk_size_mult;
...
int  cblk_mc_clone(cflsh_chunk_t *chunk,int mode, int flags)
{
    ...
    old_mc_handle = chunk->master.mc_handle;
    ...
    rc = mc_register(chunk->master_name,chunk->contxt_handle,chunk->mmio_mmap,
		&(chunk->master.mc_handle));
    ...
    rc = mc_clone(chunk->master.mc_handle,old_mc_handle,chunk_flags);
...
void  cblk_chunk_free_mc_device_resources(cflsh_chunk_t *chunk)
{
    ...
    rc = mc_close(chunk->master.mc_handle,chunk->master.sisl.resrc_handle);
...
void cblk_check_os_adap_err(cflsh_chunk_t *chunk)
{
    ...
    cblk_notify_mc_err(chunk,CFLSH_BLK_NOTIFY_AFU_FREEZE,NULL,NULL);
			  ...
			  struct request_sense_data *sense_data)
{
	...
	notify.cmd_timeout.res_hndl = chunk->master.sisl.resrc_handle;
	break;
	...
	notify.scsi_sense.res_hndl = chunk->master.sisl.resrc_handle;
	if (sense_data) {


	    bcopy(sense_data,notify.scsi_sense.data,
    ...
    rc = mc_notify(chunk->master.mc_handle,&notify);
